...
class HeadHunter(FasterRCNN):
    ...
    def __preprocess_im(self, im):
        """
        from (H, W, C) np.uint8 → (1, C, H, W) torch.float32
        """
        if not isinstance(im, np.ndarray):
            raise ValueError("Wrong image type.")

        # (H, W, C) → (C, H, W), float32로 변환 후 0~1 정규화
        torched_im = torch.from_numpy(im).permute(2, 0, 1).float() / 255.0

        return torch.unsqueeze(torched_im.to(self.__device), 0)
    @torch.no_grad()
