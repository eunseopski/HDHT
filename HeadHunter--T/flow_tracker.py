...
class Tracker:
    ...
    def __init__(self, obj_detector, tracker_cfg, tracktor_cfg, motion_cfg,
                 im_shape, save_dir=None, save_frames=True,
    ...
    def track_match(self):
        ...
        f_all_scores, f_all_pos, f_best_scores, f_best_pos, match_ids = self.filter_regressed_tracks(all_scores,
            ...
            # NMS 예외처리
        if isinstance(f_best_pos, torch.Tensor) and f_best_pos.ndim == 1:
            f_best_pos = f_best_pos.unsqueeze(0)
        elif f_best_pos.numel() == 0:
            f_best_pos = torch.empty((0, 4), dtype=torch.float32, device=self.device)

        if isinstance(f_best_scores, torch.Tensor) and f_best_scores.ndim == 0:
            f_best_scores = f_best_scores.unsqueeze(0)
        elif f_best_scores.numel() == 0:
            f_best_scores = torch.empty((0,), dtype=torch.float32, device=self.device)

        if f_best_pos.ndim != 2 or f_best_pos.shape[1] != 4 or f_best_pos.shape[0] == 0:
            keep_tracks = torch.tensor([], dtype=torch.long)
        else:
        ...
        if isinstance(self.det_scores, np.ndarray):
            det_scores_np = self.det_scores
            if det_scores_np.ndim == 0:
                det_scores_np = np.array([det_scores_np])
        else:
            det_scores_np = np.array([])

        if isinstance(self.boxes, np.ndarray) and self.boxes.size > 0:
            det_pos_gpu = torch.tensor(self.boxes, dtype=torch.float32, device=self.device)
        else:
            det_pos_gpu = torch.empty((0, 4), dtype=torch.float32, device=self.device)

        if nms_pos.numel() == 0:
            combined_pos = det_pos_gpu
        elif det_pos_gpu.numel() == 0:
            combined_pos = nms_pos
        else:
            combined_pos = torch.cat([nms_pos, det_pos_gpu], dim=0)

        nms_score_tensor = torch.ones(nms_scores.shape[0], dtype=torch.float32, device=self.device)
        det_score_tensor = torch.tensor(det_scores_np, dtype=torch.float32, device=self.device)

        if det_score_tensor.ndim == 0:
            det_score_tensor = det_score_tensor.unsqueeze(0)

        if nms_score_tensor.numel() == 0:
            combined_scores = det_score_tensor
        elif det_score_tensor.numel() == 0:
            combined_scores = 2 * nms_score_tensor
        else:
            combined_scores = torch.cat([2 * nms_score_tensor, det_score_tensor], dim=0)

        if isinstance(self.det_scores, np.ndarray):
            if self.det_scores.ndim == 0:
                self.det_scores = np.array([self.det_scores])
        elif np.isscalar(self.det_scores):  # float이나 int 같은 경우
            self.det_scores = np.array([self.det_scores])
        else:
            self.det_scores = np.array([])
    ...
    def step(self, blob):
        ...
        if refined_det.shape[0] == 0:
            self.boxes = np.empty((0, 4))  # 빈 박스
            self.det_scores = np.empty((0,))
        else:
            self.boxes, self.det_scores = refined_det[:, :4], np.squeeze(refined_det[:, 4:])
